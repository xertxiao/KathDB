...
def _phys_exec_rule(image_detail_low: bool = True, phy_opt: bool = True) -> str:
    ...
    rule = (
        ...
        + "Execution cost is the total tokens over all model calls. An attached image "
        "costs hundreds to thousands of tokens per call, so the biggest saving is sending "
        "each item to the model as few times as possible: read or classify it once and "
        "reuse the answer. Before writing the body, count how many calls your code makes "
        "and how large each is, as a function of the input row counts, and pick the "
        "construction with the smallest total that is still correct. Filter, join, and "
        "deduplicate in code before calling the model; call the model once per distinct "
        "item; put cheap text into the prompt instead of re-sending an image; reuse an "
        "attribute you already extracted. Stop early only when every skipped row could "
        "neither appear in nor change the output (a LIMIT reached, a per-key predicate "
        "already decided); if the output must list every qualifying row or pair, there "
        "is no early exit. Never let tokens grow with the product of two inputs (judging "
        "every pair) when one pass over one input, then matching in code, answers the "
        "query. Matching in code means exact or normalized equality; substring, shared "
        "words, or edit distance are not the model's judgement. When the match itself "
        "needs judgement, the model makes it. Code takes over a judgement only when it "
        "is exact (e.g., equality on a stored column), never by guessing from words in "
        "free text."
    )
    rule += (
...
def _logical_optimization_objective_block(
    ...
    return (
        ...
        "(model calls × prompt+completion length). If the information it infers is "
        "already stored in a column, read the column instead of calling the model; call "
        "the model once per distinct input value and map the answer back; send the "
        "shortest prompt that settles the question; when a text column can decide a row, "
        "use the cheaper text call and send the image only for rows the text cannot "
        "decide. Implement exactly this operator (every input row it is asked about, its "
        "declared output columns) and do not absorb upstream or downstream steps into it; "
        "which operators are fused is decided by the plan optimizer, not here.\n\n"
        "The output must be semantically equal to the straightforward implementation "
        "(row order and dropped intermediate columns do not matter) and must match the "
        "downstream schema exactly. A shortcut based on values you saw keeps the model "
        "call as the fallback for the rows it does not cover.\n"
    )
...
def _fused_objective_block(image_detail_low: bool = True, phy_opt: bool = True) -> str:
    ...
    return (
        "## Fuse These Steps\n"
        "Write one function whose result equals running the sub-steps above in order, "
        "and make it as cheap as possible in tokens and model calls. Any rewrite that "
        "reduces either is fine: send only the fields a call needs, call the model once "
        "per distinct value, filter rows on cheap columns before an AI call, reorder work "
        "so a downstream limit, exists, or threshold is reached sooner and stop there. "
        "You are bound to each sub-step's result, not its implementation. If a sub-step "
        "asks the model for something an existing column already stores, read the column "
        "instead. Hard limits: the output must equal the unoptimized answer and match the "
        "output schema above. Do not group, join, or compare rows on a model-extracted "
        "label as if it were exact unless that holds for any data; when correctness "
        "depends on values you cannot see, keep the model in the loop. A shortcut based "
        "on values you saw keeps the model call as the fallback for the rows it does not "
        "cover. "
